...
Entity* EntityFactory::CreateEntity(unsigned int aGID, eEntityType aType, Prism::Scene* aScene, const CU::Vector3f& aPosition
	, const CU::Vector3f& aRotation, const CU::Vector3f& aScale)
{
	...
	{
		Entity* newEntity = new Entity(aGID, myInstance->myLoadedEntityData.find(aType)->second, aScene, aPosition, aRotation
			, aScale);
		return newEntity;
	}
...
Entity* EntityFactory::CreateEntity(unsigned int aGID, eEntityType, std::string aSubType, Prism::Scene* aScene, const CU::Vector3f& aPosition,
	const CU::Vector3f& aRotation, const CU::Vector3f& aScale)
{

		if (myInstance->myLoadedSubEntityData.find(aSubType) != myInstance->myLoadedSubEntityData.end())
		{
			Entity* newEntity = new Entity(aGID, myInstance->myLoadedSubEntityData.find(aSubType)->second, aScene, aPosition, aRotation
				, aScale, aSubType);
