...
namespace Prism
{
	DeferredRenderer::DeferredRenderer()
	{
		...
		myActualParticleDepth = new Texture();
		myActualParticleDepth->InitAsDepthBuffer(windowSize.x, windowSize.y);
	...
	DeferredRenderer::~DeferredRenderer()
	{
		...
		SAFE_DELETE(myActualParticleDepth);
	...
	void DeferredRenderer::AddDecal(const CU::Vector3<float>& aPosition, const CU::Vector3<float>& aDirection, eDecalType aType)
	{
		//SET_RUNTIME(false);
		myDecal->AddDecal(aPosition, aDirection, aType);
		//SET_RUNTIME(true);
	}
